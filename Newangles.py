...
DISTANCES = ['11.0'] #Height element of Test Setup
...
#Get data
# here distance is fix, the best one 11,5 an we characterize the mechanical play around it with varius hight in distances vector around 11.5
# Out: List of all keys as data frame
def get_data_mati(file_template, keys, distance, part):
    data_dict = {dis: [] for dis in distance}
    for dis in distance:
        for key in keys:
            file = get_filename(file_template, dis, key, part)
            data_dict[dis].append(pd.read_csv(file, index_col=0))
    data_dict[dis] = get_average_df(data_dict[dis])
    return data_dict
...
#Get data
# Out: List of all keys as data frame
def get_avgg_data(file_template, keys, distance, part):
    data_lst = []
    avg = pd.DataFrame()
    for key in keys:
        file = get_filename(file_template, distance, key, part)
        df = pd.read_csv(file, index_col=0)
        mean_df = df.mean(axis=1).to_frame('mean_value')
        # Concatenate the mean DataFrame to the result
        avg = pd.concat([avg, mean_df], ignore_index=True)
    return data_lst
...
# Get data
# Out: List of all keys and distances as data frame
def get_avg_data(file_temp, keys, distances, part):
    data_lst = []
    for distance in distances:
        print(distance)
        all_data = get_average_df(get_data(FILENAME_TEMP, KEYS, distance, part))
        data = pd.concat(all_data)
        data_lst.append(data)
    return data_lst
...
#Shape_Estimation()
diz = get_avgg_data(FILENAME_TEMP, KEYS, '11.0', 'male')
